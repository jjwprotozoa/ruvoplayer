# RuvoPlayer – IPTV Player Application

<p align="center">
  <img src="./apps/web/src/assets/icons/favicon.256x256.png" alt="RuvoPlayer icon" title="Cross-platform IPTV player" />
</p>
<p align="center">
  <a href="https://github.com/jjwprotozoa/ruvoplayer/releases"><img src="https://img.shields.io/github/release/jjwprotozoa/ruvoplayer.svg?style=for-the-badge&logo=github" alt="Release"></a>
  <a href="https://github.com/jjwprotozoa/ruvoplayer/releases"><img src="https://img.shields.io/github/v/release/jjwprotozoa/ruvoplayer?include_prereleases&label=pre-release&logo=github&style=for-the-badge" /></a>
  <img alt="GitHub Workflow Status" src="https://img.shields.io/github/actions/workflow/status/jjwprotozoa/ruvoplayer/build-and-make.yaml?style=for-the-badge&logo=github">
  <a href="https://github.com/jjwprotozoa/ruvoplayer/releases"><img src="https://img.shields.io/github/downloads/jjwprotozoa/ruvoplayer/total?style=for-the-badge&logo=github" alt="Downloads"></a>
</p>

🌐 **[PWA](https://ruvoplayer.vercel.app)** | 📦 **[Desktop releases](https://github.com/jjwprotozoa/ruvoplayer/releases/latest)** | 💻 **[Source](https://github.com/jjwprotozoa/ruvoplayer)** | Upstream: [IPTVnator](https://github.com/4gray/iptvnator)

**RuvoPlayer** is a cross-platform IPTV player for live TV and streaming. It supports M3U/M3U8 playlists, Xtream Codes portals, and Stalker portals. Import playlists from remote URLs or local files, and attach EPG data in XMLTV format via URL.

RuvoPlayer is a branded fork of the open-source [IPTVnator](https://github.com/4gray/iptvnator) project, built with Electron and Angular.

⚠️ Note: RuvoPlayer does not provide any playlists or other digital content. The channels and pictures in the screenshots are for demonstration purposes only.

![RuvoPlayer: channels list, player and EPG list](./apps/website/public/screenshots/screenshot-player.webp)
...
### RuvoPlayer desktop builds

Download the latest macOS, Windows, and Linux installers from the
[RuvoPlayer release page](https://github.com/jjwprotozoa/ruvoplayer/releases/latest).

Artifacts are named `RuvoPlayer-<version>-<platform>-<arch>.<ext>` (for example
`RuvoPlayer-0.22.0-mac-arm64.dmg`).

### Web app (PWA)

Use the hosted PWA at **[ruvoplayer.vercel.app](https://ruvoplayer.vercel.app)**,
or deploy your own with the [self-hosted Docker](#self-hosted-pwa) setup. In the
PWA, open **Settings → About** for a direct link to desktop downloads.

### Upstream IPTVnator package managers

The commands below install **[IPTVnator](https://github.com/4gray/iptvnator)**
from community repositories, not this RuvoPlayer fork.
...
sessions where RuvoPlayer runs under X11 or Xwayland. Native Wayland embedding
...
If the app is in Downloads instead of Applications:
...
If you encounter the following error when launching RuvoPlayer:
...
Navigate to the RuvoPlayer installation directory and run:
...
1. Find your desktop file location:
    - **Ubuntu/Debian**: `~/.local/share/applications/ruvoplayer.desktop`
    - **System-wide**: `/usr/share/applications/ruvoplayer.desktop`

2. Edit the file and modify the `Exec` line:

    ```
    Exec=ruvoplayer --no-sandbox %U
    ```
...
```bash
ruvoplayer --no-sandbox
```

### GNU/Linux: Wayland startup failure

If RuvoPlayer exits on GNU/Linux with errors about failing to connect to
Wayland or initialize the Ozone platform, force X11/XWayland instead:

```bash
ruvoplayer --ozone-platform=x11
```
...
To build the PWA bundle (as deployed to Vercel):
...
## Upstream project and trademarks

RuvoPlayer is a fork of [IPTVnator](https://github.com/4gray/iptvnator). The
name **"IPTVnator"** and the IPTVnator logo are trademarks of the upstream
project owner. This fork uses the **RuvoPlayer** name for its own releases and
does not claim rights to the IPTVnator brand. See upstream
[TRADEMARK.md](https://github.com/4gray/iptvnator/blob/master/TRADEMARK.md) for
details.
